...
export const loadBackendModel = async (modelId: string): Promise<void> => {
  try {
    ...
    const response = await fetch(`${baseUrl}/models/${encodeURIComponent(modelId)}/load`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const generateOpenSourceCompletion = async (
  ...
  try {
    ...
    console.log(`Messages: ${messages.length}, Temperature: ${temperature}, Max tokens: ${maxTokens}`);
    ...
    if (error instanceof Error) {
      if (error.name === 'AbortError') {
        throw new Error('Request timeout - the backend may be overloaded or the model is still loading');
      } else if (error.message.includes('Failed to fetch')) {
        throw new Error('Cannot connect to the backend server. Please ensure it is running at ' + getBackendUrl());
      } else {
        throw error;
      }
    } else {
...
export const OPEN_SOURCE_CONFIG = {
  ...
  models: {
    'Orenguteng/Llama-3-8B-Lexi-Uncensored': 'Llama 3 8B Lexi Uncensored (Uncensored, creative responses, used for testing)',
  }
};
