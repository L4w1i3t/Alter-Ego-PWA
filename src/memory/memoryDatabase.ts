...
/**
 * Self-authored identity fragment. These are generated by the AI through
 * periodic self-reflection and stored to build cumulative identity over time.
 * Unlike external statistics, these represent the AI's own view of who it is.
 */
export interface StoredIdentityFragment {
  id?: number;
  persona: string;
  content: string;
  source: 'self-reflection' | 'trait-extraction';
  createdAt: string;
  /** How many total messages existed when this fragment was created */
...
class AlterEgoMemoryDB extends Dexie {
  ...
  identityFragments!: Table<StoredIdentityFragment, number>;
  ...
  constructor() {
    ...
    // Version 2: Add identity fragments table for self-authored identity evolution
    this.version(2).stores({
      messages: '++id, persona, timestamp, role, [persona+timestamp], [persona+role]',
      associations: '++id, persona, left, right, strength, [persona+left], [persona+right], [persona+left+right]',
      personaStates: '++id, &persona, lastAccessed',
      settings: 'key, updatedAt',
      identityFragments: '++id, persona, source, createdAt, [persona+createdAt]',
    });
  }
}
...
/**
 * Add a self-authored identity fragment for a persona.
 */
export async function addIdentityFragment(
  fragment: Omit<StoredIdentityFragment, 'id'>
): Promise<number> {
  return await db.identityFragments.add(fragment) as number;
}

/**
 * Get all identity fragments for a persona, newest first.
 */
export async function getIdentityFragments(
  persona: string,
  limit?: number
): Promise<StoredIdentityFragment[]> {
  let query = db.identityFragments
    .where('persona')
    .equals(persona)
    .reverse();
...
/**
 * Get the most recent identity fragment for a persona.
 */
export async function getLatestIdentityFragment(
  persona: string
): Promise<StoredIdentityFragment | undefined> {
  const fragments = await db.identityFragments
    .where('persona')
    .equals(persona)
    .reverse()
    .limit(1)
...
/**
 * Count identity fragments for a persona.
 */
export async function countIdentityFragments(persona: string): Promise<number> {
  return await db.identityFragments.where('persona').equals(persona).count();
}

/**
 * Clear all identity fragments for a persona.
 */
export async function clearIdentityFragments(persona: string): Promise<void> {
  await db.identityFragments.where('persona').equals(persona).delete();
}
...
export async function exportAllData(): Promise<{
  ...
  identityFragments: StoredIdentityFragment[];
  exportedAt: string;
}> {
  return {
    ...
    identityFragments: await db.identityFragments.toArray(),
...
export async function importData(data: {
  ...
  await db.transaction('rw', [db.messages, db.associations, db.personaStates, db.settings, db.identityFragments], async () => {
    if (data.messages) {
    ...
    if (data.identityFragments) {
      await db.identityFragments.bulkPut(data.identityFragments);
    }
  });
}
...
export async function clearAllData(): Promise<void> {
  await db.transaction('rw', [db.messages, db.associations, db.personaStates, db.settings, db.identityFragments], async () => {
    ...
    await db.identityFragments.clear();
  });
}
...
export async function getDatabaseStats(): Promise<{
  ...
  return {
    ...
    identityFragmentCount: await db.identityFragments.count(),
  };
}
