...
const testCases = [
  ...
  },
  {
    ...
    aiResponse: "Let me help clarify that for you. I understand it can be confusing.",
  ...
  },
  {
    ...
    aiResponse: "I'm sorry you're having a difficult experience. Let me help you.",
...
export function runEmotionTests() {
  ...
  testCases.forEach((testCase, index) => {    console.log(`\n--- Test Case ${index + 1} ---`);
    console.log(`User: "${testCase.userInput}"`);
    console.log(`AI: "${testCase.aiResponse}"`);
    
    const primaryEmotion = classifyConversationEmotion(testCase.userInput, testCase.aiResponse);
    const userAnalysis = analyzeEmotions(testCase.userInput);
    const responseAnalysis = analyzeEmotions(testCase.aiResponse);
    
    console.log(`User emotions: ${userAnalysis.map((e: any) => `${e.emotion}(${(e.confidence * 100).toFixed(1)}%)`).join(', ')}`);
    console.log(`Response emotions: ${responseAnalysis.map((e: any) => `${e.emotion}(${(e.confidence * 100).toFixed(1)}%)`).join(', ')}`);
    console.log(`Avatar emotion (AI response): ${primaryEmotion}`);
    console.log(`Expected user: ${testCase.expectedUserEmotion}, Expected response: ${testCase.expectedResponseEmotion}`);
    
    // Check if percentages sum to approximately 100%
    const userTotal = userAnalysis.reduce((sum: number, e: any) => sum + e.confidence, 0);
    const responseTotal = responseAnalysis.reduce((sum: number, e: any) => sum + e.confidence, 0);    console.log(`✓ User emotions total confidence: ${(userTotal * 100).toFixed(1)}%`);
    console.log(`✓ Response emotions total confidence: ${(responseTotal * 100).toFixed(1)}%`);
  });
  ...
  const edgeCases = [
    ...
    { input: "What time is it?", response: "It's 3 PM.", description: "Neutral question" },
    { input: "I LOVE this SO MUCH!!!", response: "That's fantastic!", description: "High intensity positive" },
    { input: "I hate everything", response: "I'm sorry you're feeling that way.", description: "Strong negative emotion" }
  ];
    edgeCases.forEach((testCase, index) => {
    console.log(`\n--- Edge Case ${index + 1}: ${testCase.description} ---`);
    const primaryEmotion = classifyConversationEmotion(testCase.input, testCase.response);
...
if (typeof window !== 'undefined') {
  ...
  console.log('💡 Run testEmotions() in the console to test emotion classification');
}
